...
function AddSubject() {
  ...
  // const fetchSubjects = () => {
  useEffect(() => {
    fetch("http://localhost:8000/add-subject/")
      .then((res) => res.json())
      .then((data) => {
        console.log('fetch subject..',data);
        setSubjects(data)
      })
  ...
  const handleEditSubject = (id, name) => {
    ...
    fetch("http://localhost:8000/update-subject/", {
      method: "PUT",
      headers: {
        "Content-Type": "application/json",
      },
  ...
  const handleDeleteSubject = (id) => {
    ...
    fetch(`http://localhost:8000/delete-subject/${id}`, {
      method: "DELETE",
    })
      .then((res) => {
        if (res.ok) {
